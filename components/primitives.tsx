...
export function displayLabel(value:string){return value==='NONE_ON_FILE'?'NO EVENT ON FILE':value.replaceAll('_',' ');}
export function Badge({value}:{value:string}){return <span className={`badge ${value.toLowerCase().replaceAll('_','-').replaceAll(' ','-')}`}>{displayLabel(value)}</span>;}
...
export function LifeState({life,at}:{life:Lifecycle;at?:string}){return life.deadline?<Countdown deadline={life.deadline} compact at={at}/>:<span className="muted">NO EVENT ON FILE</span>;}
export function Value({value}:{value:Json|undefined}){if(value===null || value===undefined)return <span className="no-data">NO DATA</span>;if(typeof value==='object')return <pre className="json-value">{JSON.stringify(value,null,2)}</pre>;return <span className="break-anywhere">{typeof value==='string'?displayLabel(value):String(value)}</span>;}
